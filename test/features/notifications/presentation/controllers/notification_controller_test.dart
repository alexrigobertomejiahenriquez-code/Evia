...
void main() {
  ...
  group('NotificationController', () {
    ...
    test('loadNotifications captures service error and exposes error',
        () async {
    ...
    test('addNotification adds and updates items and clears error on success',
        () async {
      ...
      await fake
          .saveNotifications([]); // ensure fake has an initially empty storage

      final item = NotificationItem(
          id: '10', title: 'New', body: 'New body', date: date2);
    ...
    test('markRead updates read state correctly', () async {
      final a = NotificationItem(
          id: 'a', title: 'A', body: 'A', date: date1, read: false);
    ...
    test('markAllRead marks all notifications as read', () async {
      ...
      final b = NotificationItem(
          id: 'b', title: 'B', body: 'B', date: date2, read: false);
      final fake = FakeNotificationService(items: [a, b]);
      final controller = NotificationController(service: fake);
    ...
    test('successful operation clears previous error', () async {
      ...
      fake.throwOnGet = false;
      await fake.saveNotifications(
          [NotificationItem(id: 'x', title: 'X', body: 'X', date: date1)]);
...
class FakeNotificationService extends NotificationServiceLocal {
  ...
  FakeNotificationService({
    ...
    this.delayMs = 0,
  }) : _items = List<NotificationItem>.from(items);
